...
$admin_from = trim($_POST["admin_from"]);

$message = "";

foreach ( $_POST as $key => $value ) {
	if ( $value != "" && $key != "project_name" && $key != "admin_email" && $key != "form_subject" && $key != "admin_from") {
		$message .= "
...
'From: '.adopt($project_name).' <'.$admin_from.'>' . PHP_EOL .
'Reply-To: '.$admin_from.'' . PHP_EOL;
